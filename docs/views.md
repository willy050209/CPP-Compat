...
| 視圖物件 | 類別樣板型別 | 說明與標準版本 |
| :--- | :--- | :--- |
| `compat::views::empty<T>` | `empty_view<T>` | 產生元素個數永遠為 0 的唯讀空視圖 (C++20)。滿足 `borrowed_range`。 |
| `compat::views::single(x)` | `single_view<T>` | 產生僅包含單一元素 `x` 的視圖 (C++20)。透過 [`enable_borrowed_range`](ranges.md#3-自訂點變數樣板-compatrangesenable_borrowed_ranger) 約束，持有實體值時預設為非借用，杜絕臨時物件迭代器懸空。 |
